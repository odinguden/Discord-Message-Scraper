...
secretTokenMainBot = os.getenv(
    "SECRET_TOKEN_MAIN"
)  # used for scraping with the main bot
...
async def write_channel_messages_to_file(
    channel,
):  # Writes all messages from a channel to a csv file
    df = pd.DataFrame(
        columns=["message", "author", "authorID", "channel", "messageId", "timestamp"]
    )
